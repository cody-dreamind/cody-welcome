...
## Příloha CW: API integrace bez tajných tunelů a datového bordelu

Integrace jsou zvláštní druh produktové magie. Na demo vypadají krásně: klikneš, data se přesunou, zákazník se usměje a obchodník má chuť objednat dort. V provozu ale špatná integrace často znamená nekonečný zdroj chyb, duplicit, tichých výpadků a bezpečnostních otázek, které nikdo nechtěl otevřít, protože „vždyť je to jen webhook“.

Privacy-first integrace má jiný cíl než jen „napojit to“. Má přenést minimum potřebných dat, jasně říct, kdo je za co odpovědný, umožnit audit a jít vypnout bez bourání celého produktu. Jinak z integrace není most, ale tajný tunel pod hradem, kde občas zmizí zákaznická data a někdo tvrdí, že to je feature.

### CW.1 Začni účelem, ne endpointem

První otázka nemá být: „Má ten systém API?“ Lepší otázka je: „Jaké rozhodnutí nebo workflow má integrace zlepšit?“ API dokumentace je nástroj, ne strategie. Když nevíš, proč data tečou, budeš jen přesouvat chaos rychleji.

U každé integrace napiš jednu větu:

> Integrace mezi **[systém A]** a **[systém B]** pomáhá **[komu]** udělat **[konkrétní práci]** bez **[ručního kroku / rizika / zpoždění]**.

Příklady:

- Integrace mezi poptávkovým formulářem a CRM pomáhá obchodníkovi zpracovat nové leady bez ručního přepisování e-mailů.
- Integrace mezi billingem a zákaznickým portálem pomáhá supportu vidět stav předplatného bez přístupu do platebního systému.
- Integrace mezi helpdeskem a produktovým backlogem pomáhá týmu přenášet opakované problémy do rozhodování bez kopírování osobních dat.

Pokud takovou větu neumíš napsat, integraci zatím nestav. Možná potřebuješ export, interní report nebo lepší proces, ne další propojení systémů.

### CW.2 Udělej datovou mapu dřív než první request

Integrace bez datové mapy je jako poslat kurýra s krabicí bez adresy a doufat, že má dobrý den. Před implementací si napiš, co přesně teče, odkud, kam, proč a jak dlouho to bude v cílovém systému žít.

Minimální datová mapa:

- **Zdroj:** systém, tabulka, formulář, webhook nebo soubor.
- **Cíl:** konkrétní systém a objekt, který data přijímá.
- **Pole:** název, typ, význam a zda jde o osobní údaj.
- **Účel:** proč je pole potřeba pro daný workflow.
- **Směr:** jednosměrná synchronizace, obousměrná synchronizace nebo jednorázový přenos.
- **Retence:** kdy se data mažou nebo anonymizují v cíli.
- **Vlastník:** člověk nebo role, která rozhoduje při konfliktu.

Praktické pravidlo: každé pole v integraci musí mít obhajobu. „Mohlo by se hodit“ není obhajoba, to je začátek datového sklepa s kobercem přes dveře.

### CW.3 Preferuj jednosměrný tok, dokud nepotřebuješ víc

Obousměrná synchronizace zní profesionálně, ale v malém týmu často přináší víc problémů než hodnoty. Najednou řešíš konflikty, časové pořadí změn, částečné výpadky a otázku, který systém je pravda. A samozřejmě to někdo objeví v pátek v 16:48.

Začni jednoduše:

- **Jednosměrný push:** systém A pošle událost do systému B.
- **Jednosměrný pull:** systém B si v intervalu vyžádá změny ze systému A.
- **Ruční potvrzení:** uživatel před přenosem vidí, co se stane.
- **Stavová fronta:** změny čekají na zpracování a mají jasný výsledek.

Obousměrnou synchronizaci zvaž až tehdy, když máš jasně určený zdroj pravdy pro každé pole, pravidlo pro konflikt, auditní log změn, možnost bezpečně zopakovat selhaný přenos a provozní odpovědnost za oba směry. Většina MVP nepotřebuje dokonalou synchronizaci. Potřebuje spolehlivě odstranit jeden bolestivý ruční krok.

### CW.4 Tokeny a oprávnění nejsou technická drobnost

API token je klíč. Ne poznámka v dokumentaci, ne „dočasná hodnota“, ne něco, co patří do sdílené tabulky. Integrace musí mít bezpečný životní cyklus přístupů od prvního testu až po vypnutí.

Dobré minimum:

- používej samostatný technický účet nebo aplikační token,
- nastav nejmenší možná oprávnění,
- odděl testovací a produkční přístupy,
- tokeny ukládej do secret manageru nebo bezpečné konfigurace,
- pravidelně kontroluj nepoužívané tokeny,
- při ukončení integrace token okamžitě zneplatni.

U B2B zákazníků přidej i provozní otázku: kdo token vytvořil, kdo ho smí rotovat a co se stane, když člověk, který ho založil, odejde z firmy? Jestli odpověď zní „to má asi Petr“, integrace není připravená na realitu. Petr je skvělý, ale není disaster recovery plán.

### CW.5 Chyby musí být viditelné a opravitelné

Nejhorší integrace nejsou ty, které spadnou nahlas. Nejhorší jsou ty, které tři týdny potichu neposílají polovinu dat a všichni se tváří, že pipeline je jen „nějak slabší“. Proto integrace potřebuje pozorovatelnost bez šmírování.

Sleduj hlavně:

- počet přijatých a zpracovaných událostí,
- počet chyb podle typu,
- stáří nejstarší čekající položky,
- poslední úspěšný přenos,
- počet opakovaných pokusů,
- ruční zásahy administrátora.

Do logů nedávej celé payloady plné osobních dat. Raději používej interní identifikátor události, typ objektu, stav, čas a krátký technický důvod chyby. Pokud potřebuješ ladit konkrétní payload, dělej to cíleně, dočasně a s jasným úklidem.

### CW.6 Integrace potřebuje vypínač a runbook

Každá integrace by měla jít pozastavit bez vypnutí celého produktu. Důvody jsou obyčejné: externí systém má výpadek, zákazník mění účet, token expiroval, dodavatel změnil API nebo tým objevil chybu v mapování dat.

Připrav si jednoduchý provozní režim:

- **Pause:** zastaví odesílání nebo zpracování nových položek.
- **Retry:** zopakuje vybrané položky po opravě příčiny.
- **Skip:** přeskočí položku s dokumentovaným důvodem.
- **Replay:** znovu přehraje události z bezpečného intervalu.
- **Disconnect:** vypne integraci a uklidí přístupy i dočasná data.

Runbook nemusí být román. Stačí stránka, kde je napsané, jak poznat problém, koho kontaktovat, jak integraci pozastavit, jak ověřit opravu a co se komunikuje zákazníkovi. Bez runbooku je integrace loterie, ve které hlavní cena je sobotní debugování.

### CW.7 Šablona integrační karty

Použij tuhle kartu pro každou důležitou integraci. Ideálně ji měj v repozitáři nebo interní znalostní bázi vedle provozní dokumentace.

```markdown
## Integrace: [název]

### Účel
- Jaké workflow zlepšuje:
- Komu pomáhá:
- Jaký ruční krok odstraňuje:
...
- Přenášená pole:
- Osobní údaje:
- Pole, která záměrně nepřenášíme:
- Retence v cíli:

### Přístupy
- Typ tokenu nebo účtu:
- Oprávnění:
- Kdo token spravuje:
- Kdy proběhne další kontrola:

### Provoz
- Jak poznáme úspěšný přenos:
- Jak poznáme selhání:
- Retry pravidlo:
- Pause/disconnect postup:

### Privacy poznámka
- Proč je rozsah dat přiměřený:
- Kde data fyzicky nebo smluvně končí:
- Jak zákazník zjistí, co integrace dělá:
```

### CW.8 Checklist: API integrace bez tajných tunelů

- [ ] Integrace má jasnou větu účelu, ne jen seznam endpointů.
- [ ] Existuje datová mapa s poli, účelem, směrem toku a retencí.
- [ ] Přenáší se jen data, která jsou nutná pro konkrétní workflow.
- [ ] Je jasné, který systém je zdroj pravdy a kdo řeší konflikt.
- [ ] Tokeny mají nejmenší oprávnění a oddělený testovací a produkční režim.
- [ ] Chyby jsou viditelné přes metriky a logy bez ukládání celých citlivých payloadů.
- [ ] Integraci lze pozastavit, zopakovat selhané položky a bezpečně odpojit.
- [ ] Zákazník srozumitelně ví, co integrace dělá s jeho daty.
